...
async def create_user(user: UserCreate):
    try:
        ...
        return JSONResponse(
            ...
            content=response(False, None, f"Error during signup: {str(e)}")
        )
...
async def login(user: UserLogin):
    ...
    try:
        ...
        print(f"Login completed in {round((end - start) * 1000)}ms")
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=response(False, None, f"Error during login: {str(e)}")
        )
...
async def reset_password(user: UserLogin):
    try:
        ...
        # Placeholder: integrate actual email service here
        return JSONResponse(
            status_code=status.HTTP_200_OK,
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content=response(False, None, f"Error during reset: {str(e)}")
        )
